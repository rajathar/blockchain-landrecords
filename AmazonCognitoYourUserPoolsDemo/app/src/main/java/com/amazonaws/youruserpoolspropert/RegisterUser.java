...
public class RegisterUser extends AppCompatActivity {
    ...
    private void init() {
        ...
        aadhar=(EditText) findViewById(R.id.aadharnum);
        aadhar.addTextChangedListener(new TextWatcher() {
            @Override
            public void beforeTextChanged(CharSequence s, int start, int count, int after) {
                if (s.length() == 0) {
                    TextView label = (TextView) findViewById(R.id.textViewaadharnum);
                    label.setText("Enter the government issued "+aadhar.getHint() );
                    aadhar.setBackground(getDrawable(R.drawable.text_border_selector));
                }
            }
        ...
        signUp.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                userInput = aadhar.getText().toString();
                if (userInput != null) {
                    if (userInput.length() > 0) {
                        userAttributes.addAttribute(AppHelper.getSignUpFieldsC2O().get(aadhar.getHint()).toString(), userInput);
                    }
                }
